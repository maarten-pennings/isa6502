...
static void cmdasm_stream( int argc, char * argv[] ) {
  ...
  if( argc==0 ) { // no arguments toggles streaming mode
    ...
    if( iix==0 ) { Serial.println(F("ERROR: unknown mnenemonic")); return; }
    // Parse operand syntax to find addressing mode
    if( argc==1 ) buf[0]='\0'; else strncpy(buf, argv[1], sizeof buf );
    int aix= isa_parse(buf);
    if( aix==0 ) { Serial.println(F("ERROR: syntax error in operand")); return; }
    ...
    if( opcode==ISA_OPCODE_INVALID ) { Serial.println(F("ERROR: instruction does not have that addressing mode")); return; }
    // Check operand size
    uint16_t op;
    int bytes= isa_addrmode_bytes(aix);
    if( bytes>1 ) { // todo: what is buf here?
      if( !cmd_parse(buf,&op) ) { Serial.println(F("ERROR: operand must be <hex>")); return; }
      if( rel_as_abs ) { op= op-(cmdasm_addr+2); if( 0x7f<op && op<0xff80 ) { Serial.println(F("ERROR: ABS address too far")); return; } op&=0xFF; }
      if( !rel_as_abs && bytes==2 && op>0xff ) { Serial.println(F("ERROR: operand must be 00..ff")); return; }
    }
    // Check
    if( argc>2 ) { Serial.println(F("ERROR: text after operand")); return; }
    // Now assemble
    ...
    if( aix==ISA_AIX_REL && PAGE(cmdasm_addr+(int8_t)op)!=PAGE(cmdasm_addr) ) Serial.println(F("INFO: branch to other page (1 cycle extra) (try - for undo)")); 
  }
