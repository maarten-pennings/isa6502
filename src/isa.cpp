...
// This file is generated by isa6502.py V7 on 2020-07-19 13:10:49
...
const char ISA_AIX_0Ea_syntax[] /* 0*/ PROGMEM = "??err??";
...
const char ISA_AIX_REL_aname [] /* 8*/ PROGMEM = "REL";
const char ISA_AIX_REL_desc  [] /* 8*/ PROGMEM = "relative to PC";
const char ISA_AIX_REL_syntax[] /* 8*/ PROGMEM = "OPC +NN";
const char ISA_AIX_ZIY_aname [] /* 9*/ PROGMEM = "ZIY";
const char ISA_AIX_ZIY_desc  [] /* 9*/ PROGMEM = "zero page, indirect, indexed with y";
const char ISA_AIX_ZIY_syntax[] /* 9*/ PROGMEM = "OPC (LL),Y";
const char ISA_AIX_ZPG_aname [] /*10*/ PROGMEM = "ZPG";
const char ISA_AIX_ZPG_desc  [] /*10*/ PROGMEM = "zero page";
const char ISA_AIX_ZPG_syntax[] /*10*/ PROGMEM = "OPC *LL";
const char ISA_AIX_ZPX_aname [] /*11*/ PROGMEM = "ZPX";
const char ISA_AIX_ZPX_desc  [] /*11*/ PROGMEM = "zero page, indexed with x";
const char ISA_AIX_ZPX_syntax[] /*11*/ PROGMEM = "OPC *LL,X";
const char ISA_AIX_ZPY_aname [] /*12*/ PROGMEM = "ZPY";
const char ISA_AIX_ZPY_desc  [] /*12*/ PROGMEM = "zero page, indexed with y";
const char ISA_AIX_ZPY_syntax[] /*12*/ PROGMEM = "OPC *LL,Y";
const char ISA_AIX_ZXI_aname [] /*13*/ PROGMEM = "ZXI";
const char ISA_AIX_ZXI_desc  [] /*13*/ PROGMEM = "zero page, indexed with x, indirect";
const char ISA_AIX_ZXI_syntax[] /*13*/ PROGMEM = "OPC (LL,X)";
...
const isa_addrmode_t isa_addrmodes[] PROGMEM = {
  /* 0*/ { ISA_AIX_0Ea_aname, 1, ISA_AIX_0Ea_desc, ISA_AIX_0Ea_syntax },
  ...
  /* 8*/ { ISA_AIX_REL_aname, 2, ISA_AIX_REL_desc, ISA_AIX_REL_syntax },
  /* 9*/ { ISA_AIX_ZIY_aname, 2, ISA_AIX_ZIY_desc, ISA_AIX_ZIY_syntax },
  /*10*/ { ISA_AIX_ZPG_aname, 2, ISA_AIX_ZPG_desc, ISA_AIX_ZPG_syntax },
  /*11*/ { ISA_AIX_ZPX_aname, 2, ISA_AIX_ZPX_desc, ISA_AIX_ZPX_syntax },
  /*12*/ { ISA_AIX_ZPY_aname, 2, ISA_AIX_ZPY_desc, ISA_AIX_ZPY_syntax },
  /*13*/ { ISA_AIX_ZXI_aname, 2, ISA_AIX_ZXI_desc, ISA_AIX_ZXI_syntax },
};
...
// Prints the addressing mode name (eg ABS) associated with `aix` to `str`. 
// This function writes at most `size` bytes to `str`.
// If the name is shorter than `minlen`, spaces will be appended until name plus spaces equals `minlen`.
// When size>0, a terminating zero will be added;
// even if the name is longer than `size` (the operand string will be truncated).
// Returns the number of characters that would be written if size were high enough (excluding the terminating 0).
// In other words, when returnvalue<size the complete name (with terminating 0) is written.
#define _s(p) (uint8_t)pgm_read_byte(p)
int isa_snprint_aname(char * str, int size, int minlen, int aix) {
  const char * name= isa_addrmode_aname(aix);
  int len=0; // number of chars printed to `str`
  // Copy name
  char c;
  while( c=_s(name), c!=0 ) { len++; name++; if(size>0) {size--; *str++=(size==0)?'\0':c; } } 
  // Write padding
  while( len<minlen ) {
...
int isa_addrmode_find(const char * aname) {
  ...
  while( lo<=hi ) {
    int mid= (lo+hi)/2;
    int cmp= strcasecmp_P(aname,isa_addrmode_aname(mid));
    if( cmp==0 ) return mid;
...
// Prints the 'operand-string' to `str`. 
// The operand-string is the operand `op` formated according to addressing mode `aix`.
// For example if `op` is '$1234', and `iax` is ISA_AIX_IND (indirect addressing), 
// the operand-string '($1234)' will be printed to `str`.
// If `op` is 0 a default string will be supplied (HHLL, LL, NN) depending on addressing mode
//
// This function writes at most `size` bytes to `str`.
// When size>0, a terminating zero will be added;
// even if the operand-string is longer than `size` (the operand string will be truncated).
// Returns the number of characters that would be written if size were high enough (excluding the terminating 0).
// In other words, when returnvalue<size the complete `fsx`-string (with padding _and_ terminating 0) is written.
#define _s(p) (uint8_t)pgm_read_byte(p)
int isa_snprint_op(char * str, int size, int aix, const char * op) {
  const char * fmt= isa_addrmode_syntax(aix);
  int len=0; // number of chars printed to `str`
  char c; // temp variable holding `*fmt`, we use this because fmt is a pointer to PROGMEM. Hence the _s() macro
  // Part 1: skip OPC (do _not_ copy from fmt to str)
  while( c=_s(fmt), c=='O'||c=='P'||c=='C'||c==' '  ) fmt++;
  // Part 2: write prefix of the operand (do copy from fmt to buf)
  while( c=_s(fmt), c!='H'&&c!='L'&&c!='N'&&c!=0 ) { len++; fmt++; if(size>0) {size--; *str++=(size==0)?'\0':c; } } // copy fmt
  // Part 3: write op (replace HLN chars by op, unless op==0)
  int HLN= c!=0;
  if( op ) {
    while( c=_s(fmt), c=='H'||c=='L'||c=='N' ) fmt++; // skip HLN
    if( HLN ) while( *op!=0 ) { len++; c=*op++; if(size>0) {size--; *str++=(size==0)?'\0':c; } } // copy op (if there was HLN)
  } else {
    while( c=_s(fmt), c=='H'||c=='L'||c=='N' ) { len++; fmt++; if(size>0) {size--; *str++=(size==0)?'\0':c; } }  // copy fmt
  }
  // Part 4: write the postfix (do copy from fmt to buf)
  while( c=_s(fmt), c!=0 ) { len++; fmt++; if(size>0) {size--; *str++=(size==0)?'\0':c; } } // copy fmt
  // Part 5: write terminating 0
  if( size>0 ) { size--; *str++='\0'; }
  // len++; // snprintf doesn't count the terminating 0
  // Returns wish length
  return len;
...
// Returns the aix matching the addressing mode syntax of operand `ops`.
// If no addressing mode syntax matches 0 is returned.
// If there is a match, `ops` is changed to only contain the operand.
// "($3,X)" -> "$3"
// "(VEC8)" -> "VEC8"
// "#>AAP" -> ">AAP"
int isa_parse(char * ops) {
  char c;
  ...
  // Skip all pre chars
  char * op0= pre0;
  while( c=*op0, c=='#'||c=='('||c=='+'||c=='*' ) op0++;
  if( op0==pre0 && (*op0=='A'||*op0=='a') && *(op0+1)==0 ) op0++; // special case ACC addressing
  int pre0len= op0-pre0;
  // Skip all operand chars
  char * post0= op0;
  while( c=*post0, c!=')'&&c!=','&&c!=0 ) post0++;
  int op0len= post0-op0;
  // Search for end of string
  char * end0= post0;
  while( c=*end0, c!=0 ) end0++;
  int post0len= end0-post0;
  // debug
  //Serial.println("0123456789");
  //Serial.println(str0);
  //Serial.print("pre0="    ); Serial.print(pre0-str0); Serial.print(", ");
  //Serial.print("op0="     ); Serial.print(op0-str0 ); Serial.print(", ");
  //Serial.print("post0="   ); Serial.print(post0-str0); Serial.print(", ");
  //Serial.print("end0="    ); Serial.print(end0-str0 ); Serial.print(", ");
  //Serial.print("pre0len=" ); Serial.print(pre0len); Serial.print(", ");
  //Serial.print("op0len="  ); Serial.print(op0len); Serial.print(", ");
  //Serial.print("post0len="); Serial.print(post0len); Serial.println();
  // Search
  for( int aix=ISA_AIX_FIRST; aix<ISA_AIX_LAST; aix++)  {
    const char * str1= isa_addrmode_syntax(aix); // in PROGMEM!
    // Strip OPC, to find the start of pre-string
    const char * pre1= str1;
    while( c=_s(pre1), c=='O'||c=='P'||c=='C'||c==' ' ) pre1++;
    // Skip all pre chars
    const char * op1= pre1;
    while( c=_s(op1), c=='#'||c=='('||c=='+'||c=='*' ) op1++;
    if( op1==pre1 && _s(op1)=='A' && _s(op1+1)==0 ) op1++; // special case ACC addressing
    int pre1len= op1-pre1;
    // Skip all operand chars
    const char * post1= op1;
    while( c=_s(post1), c!=')'&&c!=','&&c!=0 ) post1++;
    int op1len= post1-op1;
    // Search for end of string
    const char * end1= post1;
    while( c=_s(end1), c!=0 ) end1++;
    int post1len= end1-post1;
    // debug
    //Serial.println("0123456789");
    //Serial.println(f(str1));
    //Serial.print("pre1="    ); Serial.print(pre1-str1); Serial.print(", ");
    //Serial.print("op1="     ); Serial.print(op1-str1 ); Serial.print(", ");
    //Serial.print("post1="   ); Serial.print(post1-str1); Serial.print(", ");
    //Serial.print("end1="    ); Serial.print(end1-str1 ); Serial.print(", ");
    //Serial.print("pre1len=" ); Serial.print(pre1len); Serial.print(", ");
    //Serial.print("op1len="  ); Serial.print(op1len); Serial.print(", ");
    //Serial.print("post1len="); Serial.print(post1len); Serial.println();
    // Do thestrings match?
    int prematch= pre0len==pre1len && strncasecmp_P(pre0,pre1,pre0len)==0;
    int opmatch= (op0len>0) == (op1len>0);
    int postmatch= post0len==post1len && strncasecmp_P(post0,post1,post0len)==0;
    if( prematch && opmatch && postmatch ) {
      memmove(ops,op0,op0len);
      ops[op0len]=0;
      return aix;
    }
  }
  return 0; // The error aix
}
...
const isa_instruction_t isa_instructions[] PROGMEM = {
  ...
  /* 1*/ { ISA_IIX_ADC_iname, ISA_IIX_ADC_desc, ISA_IIX_ADC_help, ISA_IIX_ADC_flags, {0xbb,0x6d,0x7d,0x79,0xbb,0x69,0xbb,0xbb,0xbb,0x71,0x65,0x75,0xbb,0x61} },
  /* 2*/ { ISA_IIX_AND_iname, ISA_IIX_AND_desc, ISA_IIX_AND_help, ISA_IIX_AND_flags, {0xbb,0x2d,0x3d,0x39,0xbb,0x29,0xbb,0xbb,0xbb,0x31,0x25,0x35,0xbb,0x21} },
  /* 3*/ { ISA_IIX_ASL_iname, ISA_IIX_ASL_desc, ISA_IIX_ASL_help, ISA_IIX_ASL_flags, {0xbb,0x0e,0x1e,0xbb,0x0a,0xbb,0xbb,0xbb,0xbb,0xbb,0x06,0x16,0xbb,0xbb} },
  /* 4*/ { ISA_IIX_BCC_iname, ISA_IIX_BCC_desc, ISA_IIX_BCC_help, ISA_IIX_BCC_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x90,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /* 5*/ { ISA_IIX_BCS_iname, ISA_IIX_BCS_desc, ISA_IIX_BCS_help, ISA_IIX_BCS_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xb0,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /* 6*/ { ISA_IIX_BEQ_iname, ISA_IIX_BEQ_desc, ISA_IIX_BEQ_help, ISA_IIX_BEQ_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xf0,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /* 7*/ { ISA_IIX_BIT_iname, ISA_IIX_BIT_desc, ISA_IIX_BIT_help, ISA_IIX_BIT_flags, {0xbb,0x2c,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x24,0xbb,0xbb,0xbb} },
  /* 8*/ { ISA_IIX_BMI_iname, ISA_IIX_BMI_desc, ISA_IIX_BMI_help, ISA_IIX_BMI_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x30,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /* 9*/ { ISA_IIX_BNE_iname, ISA_IIX_BNE_desc, ISA_IIX_BNE_help, ISA_IIX_BNE_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xd0,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /*10*/ { ISA_IIX_BPL_iname, ISA_IIX_BPL_desc, ISA_IIX_BPL_help, ISA_IIX_BPL_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x10,0xbb,0xbb,0xbb,0xbb,0xbb} },
  ...
  /*12*/ { ISA_IIX_BVC_iname, ISA_IIX_BVC_desc, ISA_IIX_BVC_help, ISA_IIX_BVC_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x50,0xbb,0xbb,0xbb,0xbb,0xbb} },
  /*13*/ { ISA_IIX_BVS_iname, ISA_IIX_BVS_desc, ISA_IIX_BVS_help, ISA_IIX_BVS_flags, {0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x70,0xbb,0xbb,0xbb,0xbb,0xbb} },
  ...
  /*18*/ { ISA_IIX_CMP_iname, ISA_IIX_CMP_desc, ISA_IIX_CMP_help, ISA_IIX_CMP_flags, {0xbb,0xcd,0xdd,0xd9,0xbb,0xc9,0xbb,0xbb,0xbb,0xd1,0xc5,0xd5,0xbb,0xc1} },
  /*19*/ { ISA_IIX_CPX_iname, ISA_IIX_CPX_desc, ISA_IIX_CPX_help, ISA_IIX_CPX_flags, {0xbb,0xec,0xbb,0xbb,0xbb,0xe0,0xbb,0xbb,0xbb,0xbb,0xe4,0xbb,0xbb,0xbb} },
  /*20*/ { ISA_IIX_CPY_iname, ISA_IIX_CPY_desc, ISA_IIX_CPY_help, ISA_IIX_CPY_flags, {0xbb,0xcc,0xbb,0xbb,0xbb,0xc0,0xbb,0xbb,0xbb,0xbb,0xc4,0xbb,0xbb,0xbb} },
  /*21*/ { ISA_IIX_DEC_iname, ISA_IIX_DEC_desc, ISA_IIX_DEC_help, ISA_IIX_DEC_flags, {0xbb,0xce,0xde,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xc6,0xd6,0xbb,0xbb} },
  ...
  /*24*/ { ISA_IIX_EOR_iname, ISA_IIX_EOR_desc, ISA_IIX_EOR_help, ISA_IIX_EOR_flags, {0xbb,0x4d,0x5d,0x59,0xbb,0x49,0xbb,0xbb,0xbb,0x51,0x45,0x55,0xbb,0x41} },
  /*25*/ { ISA_IIX_INC_iname, ISA_IIX_INC_desc, ISA_IIX_INC_help, ISA_IIX_INC_flags, {0xbb,0xee,0xfe,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xe6,0xf6,0xbb,0xbb} },
  ...
  /*30*/ { ISA_IIX_LDA_iname, ISA_IIX_LDA_desc, ISA_IIX_LDA_help, ISA_IIX_LDA_flags, {0xbb,0xad,0xbd,0xb9,0xbb,0xa9,0xbb,0xbb,0xbb,0xb1,0xa5,0xb5,0xbb,0xa1} },
  /*31*/ { ISA_IIX_LDX_iname, ISA_IIX_LDX_desc, ISA_IIX_LDX_help, ISA_IIX_LDX_flags, {0xbb,0xae,0xbb,0xbe,0xbb,0xa2,0xbb,0xbb,0xbb,0xbb,0xa6,0xbb,0xb6,0xbb} },
  /*32*/ { ISA_IIX_LDY_iname, ISA_IIX_LDY_desc, ISA_IIX_LDY_help, ISA_IIX_LDY_flags, {0xbb,0xac,0xbc,0xbb,0xbb,0xa0,0xbb,0xbb,0xbb,0xbb,0xa4,0xb4,0xbb,0xbb} },
  /*33*/ { ISA_IIX_LSR_iname, ISA_IIX_LSR_desc, ISA_IIX_LSR_help, ISA_IIX_LSR_flags, {0xbb,0x4e,0x5e,0xbb,0x4a,0xbb,0xbb,0xbb,0xbb,0xbb,0x46,0x56,0xbb,0xbb} },
  ...
  /*35*/ { ISA_IIX_ORA_iname, ISA_IIX_ORA_desc, ISA_IIX_ORA_help, ISA_IIX_ORA_flags, {0xbb,0x0d,0x1d,0x19,0xbb,0x09,0xbb,0xbb,0xbb,0x11,0x05,0x15,0xbb,0x01} },
  ...
  /*40*/ { ISA_IIX_ROL_iname, ISA_IIX_ROL_desc, ISA_IIX_ROL_help, ISA_IIX_ROL_flags, {0xbb,0x2e,0x3e,0xbb,0x2a,0xbb,0xbb,0xbb,0xbb,0xbb,0x26,0x36,0xbb,0xbb} },
  /*41*/ { ISA_IIX_ROR_iname, ISA_IIX_ROR_desc, ISA_IIX_ROR_help, ISA_IIX_ROR_flags, {0xbb,0x6e,0x7e,0xbb,0x6a,0xbb,0xbb,0xbb,0xbb,0xbb,0x66,0x76,0xbb,0xbb} },
  ...
  /*44*/ { ISA_IIX_SBC_iname, ISA_IIX_SBC_desc, ISA_IIX_SBC_help, ISA_IIX_SBC_flags, {0xbb,0xed,0xfd,0xf9,0xbb,0xe9,0xbb,0xbb,0xbb,0xf1,0xe5,0xf5,0xbb,0xe1} },
  ...
  /*48*/ { ISA_IIX_STA_iname, ISA_IIX_STA_desc, ISA_IIX_STA_help, ISA_IIX_STA_flags, {0xbb,0x8d,0x9d,0x99,0xbb,0xbb,0xbb,0xbb,0xbb,0x91,0x85,0x95,0xbb,0x81} },
  /*49*/ { ISA_IIX_STX_iname, ISA_IIX_STX_desc, ISA_IIX_STX_help, ISA_IIX_STX_flags, {0xbb,0x8e,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x86,0xbb,0x96,0xbb} },
  /*50*/ { ISA_IIX_STY_iname, ISA_IIX_STY_desc, ISA_IIX_STY_help, ISA_IIX_STY_flags, {0xbb,0x8c,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0xbb,0x84,0x94,0xbb,0xbb} },
...
// In other words, when returnvalue<size the complete name (with terminating 0) is written.
#define _s(p) (uint8_t)pgm_read_byte(p)
int isa_snprint_iname(char * str, int size, int minlen, int iix) {
  const char * name= isa_instruction_iname(iix);
  int len=0; // number of chars printed to `str`
  // Copy name
  char c;
  while( c=_s(name), c!=0 ) { len++; name++; if(size>0) {size--; *str++=(size==0)?'\0':c; } } 
...
int isa_instruction_find(const char * iname) {
  ...
  while( lo<=hi ) {
    int mid= (lo+hi)/2;
    int cmp= strcasecmp_P(iname,isa_instruction_iname(mid));
    if( cmp==0 ) return mid;
    if( cmp>0 ) lo=mid+1; else hi=mid-1;
  }
...
const opcode_t isa_opcodes[] PROGMEM = {
  ...
  /*01*/ { ISA_IIX_ORA, ISA_AIX_ZXI, 6, 0 },
  /*02*/ { 0, 0, 0, 0 },
  /*03*/ { 0, 0, 0, 0 },
  /*04*/ { 0, 0, 0, 0 },
  ...
  /*11*/ { ISA_IIX_ORA, ISA_AIX_ZIY, 5, 1 },
  /*12*/ { 0, 0, 0, 0 },
  /*13*/ { 0, 0, 0, 0 },
  /*14*/ { 0, 0, 0, 0 },
  ...
  /*21*/ { ISA_IIX_AND, ISA_AIX_ZXI, 6, 0 },
  /*22*/ { 0, 0, 0, 0 },
  /*23*/ { 0, 0, 0, 0 },
  ...
  /*31*/ { ISA_IIX_AND, ISA_AIX_ZIY, 5, 1 },
  /*32*/ { 0, 0, 0, 0 },
  /*33*/ { 0, 0, 0, 0 },
  /*34*/ { 0, 0, 0, 0 },
  ...
  /*41*/ { ISA_IIX_EOR, ISA_AIX_ZXI, 6, 0 },
  /*42*/ { 0, 0, 0, 0 },
  /*43*/ { 0, 0, 0, 0 },
  /*44*/ { 0, 0, 0, 0 },
  ...
  /*51*/ { ISA_IIX_EOR, ISA_AIX_ZIY, 5, 1 },
  /*52*/ { 0, 0, 0, 0 },
  /*53*/ { 0, 0, 0, 0 },
  /*54*/ { 0, 0, 0, 0 },
  ...
  /*61*/ { ISA_IIX_ADC, ISA_AIX_ZXI, 6, 0 },
  /*62*/ { 0, 0, 0, 0 },
  /*63*/ { 0, 0, 0, 0 },
  /*64*/ { 0, 0, 0, 0 },
  ...
  /*71*/ { ISA_IIX_ADC, ISA_AIX_ZIY, 5, 1 },
  /*72*/ { 0, 0, 0, 0 },
  /*73*/ { 0, 0, 0, 0 },
  /*74*/ { 0, 0, 0, 0 },
  ...
  /*7f*/ { 0, 0, 0, 0 },
  /*80*/ { 0, 0, 0, 0 },
  /*81*/ { ISA_IIX_STA, ISA_AIX_ZXI, 6, 0 },
  /*82*/ { 0, 0, 0, 0 },
  /*83*/ { 0, 0, 0, 0 },
  ...
  /*91*/ { ISA_IIX_STA, ISA_AIX_ZIY, 6, 0 },
  /*92*/ { 0, 0, 0, 0 },
  /*93*/ { 0, 0, 0, 0 },
  ...
  /*a1*/ { ISA_IIX_LDA, ISA_AIX_ZXI, 6, 0 },
  ...
  /*b1*/ { ISA_IIX_LDA, ISA_AIX_ZIY, 5, 1 },
  /*b2*/ { 0, 0, 0, 0 },
  /*b3*/ { 0, 0, 0, 0 },
  ...
  /*c1*/ { ISA_IIX_CMP, ISA_AIX_ZXI, 6, 0 },
  /*c2*/ { 0, 0, 0, 0 },
  /*c3*/ { 0, 0, 0, 0 },
  ...
  /*d1*/ { ISA_IIX_CMP, ISA_AIX_ZIY, 5, 1 },
  /*d2*/ { 0, 0, 0, 0 },
  /*d3*/ { 0, 0, 0, 0 },
  /*d4*/ { 0, 0, 0, 0 },
  ...
  /*e1*/ { ISA_IIX_SBC, ISA_AIX_ZXI, 6, 0 },
  /*e2*/ { 0, 0, 0, 0 },
  /*e3*/ { 0, 0, 0, 0 },
  ...
  /*f1*/ { ISA_IIX_SBC, ISA_AIX_ZIY, 5, 1 },
  /*f2*/ { 0, 0, 0, 0 },
  /*f3*/ { 0, 0, 0, 0 },
  /*f4*/ { 0, 0, 0, 0 },
