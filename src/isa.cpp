...
// This file is generated by isa6502.py V4 on 2020-05-26 00:34:10
...
#define sFMT (uint8_t)pgm_read_byte(fmt)
void isa_syntax(char * buf, const char * fmt, char * op) {
  char c;
  // Part 1: OPC (do _not_ copy from fmt to buf)
  while( c=sFMT, c=='O'||c=='P'||c=='C' ) fmt++; 
  // Part 2: pre (do copy from fmt to buf)   
  while( c=sFMT, c!='H'&&c!='L'&&c!='N'&&c!=0 ) *buf++=(fmt++,c);
  // Part 3: op (replace HLN chars by op, unless op==0)
  int HLN= c!=0;  
  if( op ) {
    while( c=sFMT, c=='H'||c=='L'||c=='N' ) fmt++; // skip HLN
    if( HLN ) while( *op!=0 ) *buf++= *op++; // copy op (if there was HLN)
  } else {
    while( c=sFMT, c=='H'||c=='L'||c=='N' ) *buf++=(fmt++,c); // copy fmt
  }
  // Part 4: post (do copy from fmt to buf)
  while( c=sFMT, c!=0 ) *buf++=(fmt++,c);
  // Part 5: terminate
  *buf= 0;
}
