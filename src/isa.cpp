...
// This file is generated by isa6502.py V5 on 2020-05-26 20:07:53
...
// "OPC (LL,X)", "A3" -> " (A3,X)"
// "OPC (LL,X)", 0 -> " (LL,X)"
// buf size is 5+len(op)+1
#define _s(p) (uint8_t)pgm_read_byte(p)
void isa_unparse(char * buf, const char * fmt, char * op) {
  char c;
  // Part 1: OPC (do _not_ copy from fmt to buf)
  while( c=_s(fmt), c=='O'||c=='P'||c=='C' ) fmt++;
  // Part 2: pre (do copy from fmt to buf)
  while( c=_s(fmt), c!='H'&&c!='L'&&c!='N'&&c!=0 ) *buf++=(fmt++,c);
  // Part 3: op (replace HLN chars by op, unless op==0)
  int HLN= c!=0;
  if( op ) {
    while( c=_s(fmt), c=='H'||c=='L'||c=='N' ) fmt++; // skip HLN
    if( HLN ) while( *op!=0 ) *buf++= *op++; // copy op (if there was HLN)
  } else {
    while( c=_s(fmt), c=='H'||c=='L'||c=='N' ) *buf++=(fmt++,c); // copy fmt
  }
  // Part 4: post (do copy from fmt to buf)
  while( c=_s(fmt), c!=0 ) *buf++=(fmt++,c);
  // Part 5: terminate
  *buf= 0;
}
...
int isa_parse(char * ops) {
  ...
  if( op0==pre0 && *op0=='A' && *(op0+1)==0 ) op0++; // special case ACC addressing
