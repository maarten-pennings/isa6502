...
To store a variable in PROGMEM, add attribute `PROGMEM` (and make sure it is constant and global - not on the stack). 
...
So letting a pointer `s` point to characters in PROGMEMT is now written as
