...
def print_cpp_addrmodes() :
  ...
  print("void isa_unparse(char * buf, const char * fmt, char * op) {")
  print("  char c;")
  print("  // Part 1: OPC (do _not_ copy from fmt to buf)")
  print("  while( c=_s(fmt), c=='O'||c=='P'||c=='C' ) fmt++;")
  print("  // Part 2: pre (do copy from fmt to buf)")
  print("  while( c=_s(fmt), c!='H'&&c!='L'&&c!='N'&&c!=0 ) *buf++=(fmt++,c);")
  ...
  print("  } else {")
  print("    while( c=_s(fmt), c=='H'||c=='L'||c=='N' ) *buf++=(fmt++,c); // copy fmt")
  print("  }")
  print("  // Part 4: post (do copy from fmt to buf)")
  print("  while( c=_s(fmt), c!=0 ) *buf++=(fmt++,c);")
  ...
  print("  if( op0==pre0 && *op0=='A' && *(op0+1)==0 ) op0++; // special case ACC addressing")
...
def print_h_addrmodes() :
  ...
  print("void         isa_unparse(char*buf,const char*fmt,char*op);// fmt is a isa_addrmode_syntax(), and op an actual operand; prints to buf")
